...
$sql = "select id_usuarios,nome,id_clientes_fk from " . $bancoMBAssociados . ".usuarios where status = 'A' and email = '" . $_POST["login"] . "' and senha='" . $_POST["pass"] . "'";
//echo $sql;
...
if ($qtdeLinhasUsuairo <= 0) {
   ...
   // die ($sql." - ".count($rowProdutosUsuario));
	...
	$session->set('id_cliente', $rowLoginUsuario[0]['id_clientes_fk']);
	//$session->set('controle_login', '1');
	//echo $sql;
