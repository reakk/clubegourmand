...
if (count($row) > 0) {
	...
	echo "<table style='margin-left:120px'><tr><td>";
	echo "<div class='divTextoAutoresNumero2'> <span style='font-weight: bold;'>" . getDescricaoTipoProduto($row[0]['tipo_produto']) . "</span>" . " " . getPreposicaoTipoProduto($row[0]['tipo_produto']) . " " .
		"<span style='font-weight: bold;'>" . utf8_decode($row[0]['nome_autor']) . "</span>" . " " . ", MB Associados</div>" .
		"<div class='divTextoOcorridoEmNumero2'>" . getDescricaoOcorrencia($row[0]['tipo_produto']) . getHorarioPublicacao($row[0]['total_horas'], $row[0]['dia'], $row[0]['mes'], $row[0]['ano']) . "</span>" . "</span>" . "</div>";
	echo "</td></tr></table>";
	...
	if ($acessoAutorizado > 0) {
		echo nl2br(str_replace("186.202.62.1755", "127.0.0.1", $row[0]['descricao_artigo']));
	} else {
		echo nl2br(str_ireplace("186.202.62.1755", "127.0.0.1", $row[0]['descricao_artigo']));
	}
	echo "<br>";
	echo "<br>";
	
	//echo '<a href="http://www.mbassociados.com.br/index.php?option=com_content&view=article&id=46&id_conteudo='. $_GET["id_conteudo"] . '">Clique aqui para Acessar o Artigo </a> ';
	echo " </div>";
