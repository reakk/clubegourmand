...
$criterioBusca = $_REQUEST['receita'];
...
if (@ $criterioBusca != null) {
	$queryTot .= " and  (upper(c.descricao) like upper('%" . $criterioBusca . "%') or upper(c.nome) like upper('%" . $criterioBusca . "%') or upper(c.descricao_resumo) like upper('%" . $criterioBusca . "%')) and c.id_produto_fk = 19 ";
} else {
	$queryTot .= " and c.id_produto_fk = 19 ";
}
...
$pagina = (isset ($_POST['pag']) ? (int) $_POST['pag'] : 1);
$quantidade = 10;
...
$query .= "                      AND pp.id_portal_fk = $id_portal) ";
	...
	$query .= " and  (upper(c.descricao) like upper('%" . $criterioBusca . "%') or upper(c.nome) like upper('%" . $criterioBusca . "%') or upper(c.descricao_resumo) like upper('%" . $criterioBusca . "%')) and c.id_produto_fk = 19";
...
if ($totalPagina > 1) {
	...
	if($pagina!=1){
		...
		echo "<a href='#' onclick='goPagePesquisaSiteReceita(" . $anterior . ");' class='linkPaginacao' style='TEXT-DECORATION: none;'> [Anterior] </a>";
	}
	...
	for ($i = 1; $i <= $totalPagina; $i++) {
		...
		if ($i == $pagina) {
			...
			echo "<a href='#' onclick='goPagePesquisaSiteReceita($i);'  class='linkPaginacao' style='TEXT-DECORATION: none;'> $i </a>";
	...
	if($pagina != $totalPagina){
		echo "<a href='#' onclick='goPagePesquisaSiteReceita(" . $proxima . ");' class='linkPaginacao' style='TEXT-DECORATION: none;'> [Pr&oacute;xima] </a>";
